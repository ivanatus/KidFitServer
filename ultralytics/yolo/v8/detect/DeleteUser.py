...
# Enter values of user to be removed - username and password needed
while True:
...
while True:
    ...
    elif password != user_row["password"].values[0]:
        print("Incorrect password.")
    else:
        break

# Delete user values
df = df[~((df["username"].str.lower() == username.lower()) & (df["password"] == password))]
df = df.astype(str)
